      ...
      {services.map((service) => {
        ...
        return (
          <button
            ...
            className={`
              relative p-6 rounded-2xl text-start transition-all duration-300 flex flex-col gap-4 overflow-hidden group
              ${isActive 
                ? 'bg-brand-primary text-white shadow-card-hover scale-[1.02]' 
                : 'bg-white border border-slate-100 hover:border-blue-200 hover:shadow-card'
              }
            `}
          >
            {/* Background Gradient for Active State */}
            {isActive && (
                <div className="absolute inset-0 bg-gradient-to-br from-brand-primary to-[#0f172a]"></div>
            )}
            
            {/* Decorative Highlight */}
            {isHighlight && !isActive && (
               <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-brand-gold/10 to-transparent rounded-bl-full pointer-events-none"></div>
            )}
            ...
            <div className={`
              ...
              ${isActive 
                  ...
                  : isHighlight ? 'bg-brand-gold/10 text-brand-gold' : 'bg-brand-surface text-slate-500 group-hover:bg-blue-50 group-hover:text-brand-accent'
              }
            `}>
            ...
            <div className="relative z-10">
              <h3 className={`font-bold text-base mb-1 font-heading ${isActive ? 'text-white' : 'text-brand-primary group-hover:text-brand-accent transition-colors'}`}>
                {service.title}
              </h3>
              <p className={`text-xs leading-relaxed font-medium ${isActive ? 'text-blue-200' : 'text-slate-500'}`}>
                {service.desc}
              </p>
            </div>
